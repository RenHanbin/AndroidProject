...
public class MajorServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("getMajorList".equals(remark)) {
			...
			getMajorHotList(request,response);
	...
	//热门专业
	private void getMajorHotList(HttpServletRequest request, HttpServletResponse response) {
	...
	private void getMajorList(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		MajorDao majorDao=new MajorDao();
		List<MajorBean> majorList=majorDao.getMajorWorkList();
		JSONArray jsonArray=new JSONArray();
		for(int i=0;i<majorList.size();i++) {
			JSONObject object=new JSONObject();
			object.put("majorId",majorList.get(i).getMajorId());

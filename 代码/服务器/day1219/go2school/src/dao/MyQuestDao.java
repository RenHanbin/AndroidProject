...
public class MyQuestDao{
	...
	try {
		...
		while(rs.next()) {
			...
			question.setQuestionDescribe(rs.getString(3));
			question.setQuestionTime(rs.getString(4));
	...
	public List<QuestionBean> getQuestionByFollowedUserId(int followedUserId) {
		...
		try {
			...
			while(res.next()) {
				...
				questionbean.setQuestionTime(res.getString("question_time"));
				questionbean.setQuestionDescribe(res.getString("question_describe"));

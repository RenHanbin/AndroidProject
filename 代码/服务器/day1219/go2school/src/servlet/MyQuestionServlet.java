...
//		int userId=Integer.parseInt(request.getParameter("userId"));
		...
		if("getMyQuestionList".equals(remark)) {
			...
			for(int i=0;i<questionList.size();i++) {
				...
				object.put("questionDiscribe",questionList.get(i).getQuestionDescribe());
			...
			for(int i=0;i<followList.size();i++) {
				...
				List<QuestionBean> questionList =new MyQuestDao().getQuestionByFollowedUserId(followedUserId);
			...
			for(int m=0;m<questionAllList.size();m++) {
				...
				object.put("questionDescribe", questionAllList.get(m).getQuestionDescribe());
				object.put("questionTime", questionAllList.get(m).getQuestionTime());
				object.put("userImg", questionAllList.get(m).getQuestionUser().getUserImage());
